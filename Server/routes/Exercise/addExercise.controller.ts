...
export const addExerciseToExercises = async(req: Request, res: Response) =>{
    ...
    const {type,name,muscle, difficulty, instructions,userid, equipment } = req.body

    
    const {data, error}: any= await databaseQuery.selectWhere(supabase, 'Exercises', 'name', name, 'name');  
    if(error){
        console.log("Error selecting from Exercises table!")
        return res.status(400).json({mssg:"Error selecting from Exercises table!"});
    }
    if(data.length == 0){
        const {error}: any =  await databaseQuery.insert(supabase, 'Exercises', {type,name, muscle, difficulty, instructions, userid, equipment})
        if(error){
            ...
            return res.status(200).json({mssg:"Exercise has been successfully added"})
        }

    }
    else{
        console.log("Exercise is already in the Exercises table!")
        return res.status(200).json({mssg:"Exercise is already in the Exercises table!"})
    }
} 