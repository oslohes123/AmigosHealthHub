import { View, SafeAreaView, Text, Dimensions, ScrollView, TouchableOpacity } from 'react-native'
import themeContext from '../../../theme/themeContext';
import { useState, useContext, useEffect } from 'react';
import { useGetWorkoutHistory } from '../hooks/useGetWorkoutHistory';
    ...
    return (
        <SafeAreaView style={{flex: 1, justifyContent: 'space-evenly', maxHeight: screenHeight, alignItems:'center', paddingVertical: 10, backgroundColor: theme.background}}>
            ...
            {results && results.map((item) => (
                <TouchableOpacity key={item.workoutname} onPress={() => {
                    // navigation.navigate("Workout Plan Information", item)
                    console.log(`completed workout pressed: ${JSON.stringify(item)}`)
                }}> 
                    <Text style={[styles.testText, {borderColor: theme.color, color: theme.color}]}>{item.workoutname}</Text>
                </TouchableOpacity>
            ))}
